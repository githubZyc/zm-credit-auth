/*
 * Copyright 2013-2018 Tibet CITIC Guoan Real Estate Project Management Co., Ltd. Beijing Branch
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.guoanjia.common.credit.function.config;

import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;
import lombok.experimental.Accessors;
import org.apache.commons.lang3.StringUtils;

import java.io.Serializable;

/**
 * 阿里配置类
 *
 * @author AsherLi
 * @version 1.0.00
 */
@Data
@Builder
@Accessors(chain = true)
@AllArgsConstructor
@NoArgsConstructor
public class AliConfig implements Serializable {

    private static final long serialVersionUID = 322178352519836061L;

    /**
     * 应用APPID
     */
    private String appId;

    /**
     * 商户私有Key
     */
    private String privateKey;

    /**
     * 支付宝公有Key
     */
    private String aliPublicKey;

    /**
     * 传输格式
     */
    private String format;

    /**
     * 传输编码
     */
    private String charset;

    /**
     * 网关地址
     */
    private String gatewayUrl;

    /**
     * 验签类型
     */
    private String signType;

    /**
     * 数据加密Key
     */
    private String encryptKey;

    /**
     * 数据加密类型
     */
    private String encryptType;

    /**
     * 加载配置参数并校验参数
     *
     * @param pro 配置参数
     * @return 配置类
     */
    AliConfig setProperties(AliProperties pro) {

        if (StringUtils.isEmpty(pro.getGatewayUrl())) {
            throw new NullPointerException("gatewayUrl 不能为 NULL!");
        }
        if (StringUtils.isEmpty(pro.getAppId())) {
            throw new NullPointerException("appid 不能为 NULL!");
        }
        if (StringUtils.isEmpty(pro.getPrivateKey())) {
            throw new NullPointerException("privateKey 不能为 NULL!");
        }
        if (StringUtils.isEmpty(pro.getFormat())) {
            throw new NullPointerException("format 不能为 NULL!");
        }
        if (StringUtils.isEmpty(pro.getCharset())) {
            throw new NullPointerException("charset 不能为 NULL!");
        }
        if (StringUtils.isEmpty(pro.getAliPublicKey())) {
            throw new NullPointerException("aliPublicKey 不能为 NULL!");
        }
        if (StringUtils.isEmpty(pro.getSignType())) {
            throw new NullPointerException("signType 不能为  NULL!");
        }

        this.appId = pro.getAppId();
        this.privateKey = pro.getPrivateKey();
        this.aliPublicKey = pro.getAliPublicKey();
        this.format = pro.getFormat();
        this.charset = pro.getCharset();
        this.gatewayUrl = pro.getGatewayUrl();
        this.signType = pro.getSignType();
        this.encryptKey = pro.getEncryptKey();
        this.encryptType = pro.getEncryptType();
        return this;
    }
}
